...
function renderBonus(){
  ...
  $('bonusContext').textContent=week===1?'Week 1 · Final · 130 teams alive · max favorite 9 points':week===2?'Week 2 · Final · 112 teams alive · max favorite 8 points':`Week ${week} · Current · 111 teams alive · max favorite ${DATA.bonusMax} points`;
  ...
  else{const currentEntries=DATA.entries?.length?DATA.entries:DATA.history?.[2]?.entries||[];const arr=currentEntries.filter(e=>e.name!=='HH & PABLO'&&!REMOVED_ENTRY_IDS.has(e.id)).map(e=>{const prior=priorBonusStatus(e.id);const hasPick=!!e.bonus?.gameId;const s=prior==='eliminated'?'eliminated':hasPick?bonusStatus(e):'pending';return{e,s,prior}}).sort((a,b)=>{const order={alive:0,live:1,pending:2,eliminated:3};return order[a.s]-order[b.s]||a.e.id-b.e.id});const h2=DATA.history?.[2];const eligible=h2?.rows?.length?new Set(h2.rows.filter(r=>r.bonus==='alive'&&!REMOVED_ENTRY_IDS.has(r.id)).map(r=>r.id)).size:arr.filter(x=>x.prior!=='eliminated').length;$('bonusAlive').textContent=`${eligible} alive`;$('bonusList').innerHTML=arr.map(({e,s,prior})=>{const sub=prior==='eliminated'?'Out Week 1':e.bonus?.gameId?esc(e.bonus.team):'Not loaded · alive after Week 1';const label=prior==='eliminated'?'OUT':s==='alive'?'ALIVE':s==='live'?'LIVE':'PENDING';return `<div class="bonus-row"><div>${starButton(e)}<div class="bonus-copy"><div class="bonus-name">${esc(e.name)}</div><div class="bonus-pick">${sub}${e.autoPick?' · auto-pick':''}</div></div></div><div class="${label==='OUT'?'eliminated':label==='ALIVE'?'alive':''}">${label}</div></div>`}).join('')}
  bindStars($('bonusList'));
}
