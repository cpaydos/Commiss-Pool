...
function classifyPick(p,g){
  const sc=state.testScores[g.id]||state.scores[g.id];
...
async function refreshScores(){
  ...
  }catch(err){console.error(err);setFeed('Live feed unavailable — test mode is available','bad');render();}
}
function setFeed(t,cls){$('feedStatus').textContent=t;$('feedIndicator').className='status-dot '+cls}
function openTest(){const games=DATA.games.filter(g=>DATA.entries.some(e=>e.picks.some(p=>p.gameId===g.id)||e.bonus.gameId===g.id));$('testGameList').innerHTML=games.map(g=>{const s=state.testScores[g.id]||{};return `<div class="test-game"><label>${esc(g.away)} @ ${esc(g.home)}</label><input data-test="${g.id}:away" type="number" placeholder="A" value="${s.awayScore??''}"><input data-test="${g.id}:home" type="number" placeholder="H" value="${s.homeScore??''}"></div>`}).join('');$('testModal').classList.remove('hidden')}
function applyTests(){const next={};document.querySelectorAll('[data-test]').forEach(i=>{const [id,side]=i.dataset.test.split(':');const row=next[id]||(next[id]={status:'final'});if(i.value!=='')row[side+'Score']=Number(i.value)});for(const [id,s] of Object.entries(next)){if(s.awayScore==null||s.homeScore==null)delete next[id]}state.testScores=next;localStorage.setItem('commiss_test_scores',JSON.stringify(next));$('testModal').classList.add('hidden');render()}
function clearTests(){state.testScores={};localStorage.removeItem('commiss_test_scores');$('testModal').classList.add('hidden');render()}
...
$('search').oninput=renderLeaderboard;$('statusFilter').onchange=renderLeaderboard;$('sportFilter').onchange=renderGames;$('gameFilter').onchange=renderGames;$('refreshBtn').onclick=refreshScores;$('testModeBtn').onclick=openTest;$('applyTests').onclick=applyTests;$('clearTests').onclick=clearTests;document.querySelectorAll('[data-close]').forEach(x=>x.onclick=()=>$('entryModal').classList.add('hidden'));document.querySelectorAll('[data-close-test]').forEach(x=>x.onclick=()=>$('testModal').classList.add('hidden'));
